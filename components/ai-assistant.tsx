...
export default function AiAssistant({ dashboard = false, scanId = null, publicLocale = "nl" }: Props) {
  ...
  return (
    <>
      ...
      {open && (
        <div className={dashboard ? "pointer-events-none fixed inset-0 z-[120] flex items-end justify-end p-3 pb-[calc(96px+env(safe-area-inset-bottom))] sm:p-5" : "fixed inset-0 z-[120] flex items-end justify-end bg-black/50 p-0 backdrop-blur-sm sm:items-center sm:p-4"}>
          <div className={`flex w-full max-w-md flex-col overflow-hidden border border-white/10 bg-[#101B2D] shadow-2xl ${dashboard ? "pointer-events-auto h-[min(520px,62dvh)] rounded-3xl sm:h-[min(680px,90vh)]" : "h-[min(680px,100dvh)] rounded-t-3xl sm:h-[min(680px,90vh)] sm:rounded-3xl"}`}>
