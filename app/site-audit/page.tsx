...
export default function SiteAuditPage(){
  ...
  return <main className="min-h-screen bg-[#050816] text-white">
    <nav className="border-b border-white/10 bg-[#050816]/95 px-5 py-4">
      <div className="mx-auto flex max-w-7xl items-center justify-between">
        <a href="/" className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-xl bg-cyan-300 text-xs font-black text-slate-950">RF</span><span className="font-bold">RankFix <span className="text-cyan-300">AI</span></span></a>
        <div className="text-xs text-slate-500">Site-wide deterministic audit</div>
      </div>
    </nav>

    <section className="mx-auto max-w-7xl px-5 py-10 lg:px-8">
      <div className="max-w-3xl">
        <div className="text-xs font-bold uppercase tracking-[.2em] text-cyan-300">Pro Audit</div>
        <h1 className="mt-3 text-4xl font-black tracking-tight sm:text-5xl">Analyseer je hele website.</h1>
      ...
      <div className="mt-8 rounded-3xl border border-white/10 bg-white/[.035] p-3">
        <div className="flex flex-col gap-3 lg:flex-row">
          <input value={url} onChange={e=>setUrl(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")run()}} placeholder="https://jouwdomein.nl" className="min-w-0 flex-1 rounded-2xl bg-black/20 px-5 py-4 text-sm outline-none placeholder:text-slate-600"/>
          <select value={mode} onChange={e=>setMode(e.target.value)} className="rounded-2xl border border-white/10 bg-[#0a1020] px-4 py-4 text-sm outline-none">
    ...
    {audit&&<section className="mx-auto max-w-7xl px-5 pb-20 lg:px-8">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        ...
        ].map(([name,score,grade])=><div key={name as string} className="rounded-3xl border border-white/10 bg-white/[.035] p-5"><div className="text-xs uppercase tracking-widest text-slate-500">{name}</div><div className="mt-3 text-4xl font-black text-cyan-300">{score}</div>{grade&&<div className="mt-1 text-xs text-slate-500">Grade {grade}</div>}</div>)}
      </div>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[1fr_300px]">
        <div className="rounded-3xl border border-white/10 bg-white/[.035] p-6">
          ...
          <div className="mt-6 space-y-3">
            {visible.map(issue=><button key={issue.issue_id} onClick={()=>setSelected(issue)} className="w-full rounded-2xl border border-white/10 bg-black/15 p-4 text-left transition hover:border-cyan-400/20 hover:bg-white/[.04]">
        ...
        <aside className="space-y-4">
          ...
          <div className="rounded-3xl border border-cyan-400/10 bg-cyan-400/[.04] p-6"><div className="text-xs uppercase tracking-widest text-cyan-300">Engine</div><div className="mt-3 text-sm text-slate-300">Crawler {audit.crawler_version}</div><div className="mt-1 text-sm text-slate-500">Audit {audit.audit_version}</div></div>
        </aside>
      </div>
    </section>}

    {selected&&<div className="fixed inset-0 z-50 bg-black/70 p-4 backdrop-blur-sm" onClick={()=>setSelected(null)}>
      <div className="mx-auto mt-10 max-h-[85vh] max-w-3xl overflow-auto rounded-3xl border border-white/10 bg-[#090e1d] p-7 shadow-2xl" onClick={e=>e.stopPropagation()}>
        <div className="flex items-start justify-between gap-4"><div><div className="text-xs uppercase tracking-widest text-cyan-300">{selected.rule_id}</div><h2 className="mt-2 text-2xl font-black">{selected.title}</h2></div><button onClick={()=>setSelected(null)} className="rounded-xl bg-white/5 px-3 py-2 text-sm">Sluiten</button></div>
        <p className="mt-5 text-sm leading-7 text-slate-400">{selected.description}</p>
        <div className="mt-5 rounded-2xl border border-cyan-400/10 bg-cyan-400/[.04] p-4"><div className="text-xs font-bold uppercase tracking-widest text-cyan-300">Aanbeveling</div><p className="mt-2 text-sm leading-6 text-slate-300">{selected.recommendation}</p></div>
