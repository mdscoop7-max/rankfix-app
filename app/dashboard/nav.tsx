...
const links=["/dashboard","/dashboard#websites","/dashboard/scan","/dashboard/github","/dashboard/more"];
...
export default function DashboardNav({current}:{current:number}){
 ...
 return <><nav className="rf-desktop-nav" aria-label="Dashboard menu">
   <div className="rf-desktop-main">
    {links.slice(0,4).map((href,i)=><a key={href} href={href} aria-current={current===i?"page":undefined}>{mobileLabels[language][i]}</a>)}
    <a href="/dashboard/how">{desktopExtra[language][0]}</a>
    <a href="/dashboard/reviews">{desktopExtra[language][1]}</a>
    <a href="/dashboard/contact">{desktopExtra[language][2]}</a>
    <a href="/dashboard/more">{mobileLabels[language][4]}</a>
   </div>
   <a className="rf-site-link" href={base}>← {desktopExtra[language][3]}</a>
  </nav>
  <nav className="rf-nav" aria-label="Mobiele dashboardnavigatie">{links.map((href,index)=><a key={index} className={index===2?"rf-nav-scan":undefined} href={href} aria-current={current===index?"page":undefined}><span className="rf-nav-icon"><svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d={paths[index]}/></svg></span><span className="rf-nav-label">{mobileLabels[language][index]}</span></a>)}</nav></>;
}