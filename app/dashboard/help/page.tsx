...
export default function HelpPage(){
 ...
 return <main className="rf-page"><div className="rf-shell"><header className="rf-header"><a href="/dashboard" className="rf-brand">RankFix <span>AI</span></a><a href="/" className="rf-back">← Terug naar RankFix-site</a></header><div className="rf-body"><div className="rf-heading"><h1>Helpcentrum</h1><p>Alles wat je nodig hebt om RankFix te gebruiken, zonder technische kennis.</p></div><section className="rf-help-ai"><strong>🤖 Vraag RankFix AI</strong><p>Vraag bijvoorbeeld: “Waarom is mijn score lager?”, “Wat moet ik eerst oplossen?” of “Hoe koppel ik GitHub?” Gebruik de AI-knop rechtsonder.</p></section><div className="rf-help-grid">{topics.map(([title,body],i)=><button key={title} className="rf-help-topic" onClick={()=>setOpen(open===i?null:i)} aria-expanded={open===i}><span><b>{title}</b><em>{open===i?"−":"+"}</em></span>{open===i&&<p>{body}</p>}</button>)}</div><section className="rf-github-steps"><h2>GitHub in 5 eenvoudige stappen</h2><div>1. GitHub koppelen → 2. Repository kiezen → 3. RankFix maakt een fix → 4. Jij publiceert → 5. RankFix scant opnieuw</div><a href="/dashboard/github">Open GitHub-koppeling →</a></section></div><DashboardNav current={4}/></div><AiAssistant dashboard/></main>
}