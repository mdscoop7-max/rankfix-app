...
export default function HistoryPage(){
 ...
 return <main className="rf-page" lang={language}><div className="rf-shell">
  ...
  <div className="rf-body"><div className="rf-heading"><h1>Scanhistorie</h1><p>Vergelijk controles en zie of je website vooruitgaat.</p></div>
   ...
   {!loading&&!error&&trend.length>1&&<div className="rf-card" style={{marginBottom:16}}><div className="rf-section-head"><div><span>Scorehistorie</span><h2>{query||"Recente scans"}</h2></div><strong>{trend[trend.length-1]?.score}/100</strong></div><svg viewBox="0 0 100 100" role="img" aria-label="Trendlijn van de laatste scans" style={{width:"100%",height:160,overflow:"visible"}}><polyline points={trendPoints} fill="none" stroke="currentColor" strokeWidth="2" vectorEffect="non-scaling-stroke"/>{trend.map((p,i)=><circle key={i} cx={p.x} cy={p.y} r="1.8"><title>{new Date(p.date).toLocaleString(language)} · {p.score}/100</title></circle>)}</svg><p className="text-xs text-slate-500">Laatste {trend.length} scans. Een score kan veranderen door websitewijzigingen én door een nieuwe RankFix-regelversie; vergelijk daarom ook de controles zelf.</p></div>}
