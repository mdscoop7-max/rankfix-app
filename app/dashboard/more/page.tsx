...
export default function MorePage(){return <main className="rf-page"><div className="rf-shell"><header className="rf-header"><a href="/dashboard" className="rf-brand">RankFix <span>AI</span></a><a href="/" className="rf-back">← RankFix-site</a></header><DashboardNav current={4}/><div className="rf-body"><div className="rf-heading"><h1>Meer</h1><p>Alle instellingen, hulp en extra onderdelen op één plek.</p></div><div className="rf-more-grid">{items.map(([title,desc,href])=><a className="rf-more-card" href={href} key={title}><div><strong>{title}</strong><p>{desc}</p></div><span>→</span></a>)}</div></div></div></main>}