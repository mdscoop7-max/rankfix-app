...
export default function AccountSettings() {
 ...
 const t=ui[language];
 return <main className="rf-page" lang={language}><div className="rf-shell"><header className="rf-header"><a href="/dashboard" className="rf-brand">RankFix <span>AI</span></a><a href="/dashboard" className="rf-back">← Dashboard</a></header><div className="rf-body"><div className="rf-heading"><h1>{t.title}</h1><p>{t.description}</p></div><div className="rf-account-grid"><section className="rf-card"><h2>{user?.name || "…"}</h2><p>{t.email}: {user?.email || "—"}</p><p>{t.credits}: {user?.credits ?? "—"}</p></section><section className="rf-card"><label htmlFor="dashboard-language">{t.language}</label><select id="dashboard-language" value={language} onChange={e => setLanguage(e.target.value as Locale)}>{Object.entries(languageNames).map(([code,name]) => <option key={code} value={code}>{name}</option>)}</select><button onClick={save} disabled={busy}>{busy ? t.saving : t.save}</button>{message && <p role="status">{message}</p>}</section><section className="rf-card"><h2>{t.security}</h2><a href={`/account/forgot-password?lang=${language}`}>{t.password} →</a><button onClick={logout}>{t.logout}</button></section></div></div><DashboardNav current={4} /></div></main>;
}
