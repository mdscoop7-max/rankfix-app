...
export async function POST(request:Request){
  try{
    ...
    if(user.credits<10) return NextResponse.json({error:"Minimaal 10 credits nodig voor een GitHub fix."},{status:402});
    ...
    const db=getDb();
    const updated=await db.query("UPDATE users SET credits=credits-10 WHERE id=$1 AND credits>=10 RETURNING credits",[user.id]);
    if(!updated.rowCount) return NextResponse.json({error:"Onvoldoende credits."},{status:402});
    await db.query("INSERT INTO credit_transactions (user_id,amount,reason,reference_id) VALUES ($1,-10,'github_fix',$2)",[user.id,String(pr.number)]);
    return NextResponse.json({success:true,summary:generated.summary,branch,pr:{number:pr.number,url:pr.html_url,title:pr.title},creditsCharged:10,creditsRemaining:updated.rows[0].credits});
