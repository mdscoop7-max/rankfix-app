...
export default async function LocalizedInfo({ params }: { params: Promise<{ locale: string; page: string }> }) {
  ...
  return <LocaleShell locale={locale} page={page}><article className="lc-container lc-prose"><h1>{page === "about" ? about.title : page === "terms" && locale === "nl" ? termsContent.title : page === "privacy" && locale === "nl" ? privacyContent.title : title}</h1>{page === "about" ? about.sections.map((section, index) => <section key={index}><h2>{section.heading}</h2>{section.paragraphs?.map((paragraph, i) => <p key={i}>{paragraph}</p>)}{section.bullets && <ul>{section.bullets.map((item, i) => <li key={i}>{item}</li>)}</ul>}{section.values && <ol>{section.values.map((item, i) => <li key={i}>{item}</li>)}</ol>}</section>) : page === "privacy" && locale === "nl" ? <><p className="lc-note">{privacyContent.version}</p>{privacyContent.intro.map((paragraph, i) => <p key={i}>{paragraph}</p>)}{privacyContent.sections.map((section, index) => <section key={index}><h2>{section.heading}</h2>{section.paragraphs?.map((paragraph, i) => <p key={i}>{paragraph}</p>)}{section.bullets && <ul>{section.bullets.map((item, i) => <li key={i}>{item}</li>)}</ul>}{section.items && <ol>{section.items.map((item, i) => <li key={i}>{item}</li>)}</ol>}{section.closing?.map((paragraph, i) => <p key={i}>{paragraph}</p>)}</section>)}</> : page === "terms" && locale === "nl" ? <><p>{termsContent.version}</p><p className="lc-note">{termsContent.notice} Betaalde abonnementen en facturatie zijn momenteel niet beschikbaar.</p>{termsContent.sections.map((section, index) => <section key={index}><h2>{section.heading}</h2>{section.intro && <p>{section.intro}</p>}<ol>{section.items.map((item, i) => <li key={i}>{item}</li>)}</ol></section>)}</> : paragraphs.map((paragraph, index) => <p key={index}>{paragraph}</p>)}{page !== "about" && <p className="lc-note">{t.legalNote}</p>}</article></LocaleShell>;
}
