...
export async function sendScanReportEmail(report: ScanReportEmail) {
  ...
  if (!apiKey || !from) {
    throw new Error("RESEND_API_KEY en SCAN_REPORT_FROM moeten zijn ingesteld om scanrapporten te e-mailen.");
  }

  const displayName = report.name ? \` \${escapeHtml(report.name)}\` : "";
  const failed = report.checks.filter((check) => check.status !== "pass");
  const checkRows = report.checks.map((check) => \`
    <tr>
      <td style="padding:10px 8px;border-bottom:1px solid #e5e7eb;color:\${statusColor(check.status)};font-weight:700;">\${statusLabel(check.status)}</td>
      ...
      <td style="padding:10px 8px;border-bottom:1px solid #e5e7eb;">\${escapeHtml(check.fix)}</td>
    </tr>\`).join("");
...
<body style="margin:0;background:#f8fafc;font-family:Arial,Helvetica,sans-serif;color:#0f172a;">
  <div style="max-width:900px;margin:0 auto;padding:32px 18px;">
    <div style="background:#0f172a;color:#fff;border-radius:18px;padding:24px;">
      ...
      <p style="margin:0;color:#cbd5e1;">Hallo\${displayName}, hier is je scanresultaat.</p>
    </div>
    <div style="background:#fff;border-radius:18px;padding:24px;margin-top:18px;">
      ...
      <p style="margin:18px 0 0;color:#475569;">HTTP \${report.httpStatus} · server response \${report.responseTime} ms · \${failed.length} aandachtspunt(en)</p>
    </div>
    <div style="background:#fff;border-radius:18px;padding:24px;margin-top:18px;">
    ...
    <p style="font-size:12px;color:#64748b;margin:18px 4px;">Dit rapport is automatisch gegenereerd door RankFix AI. Scanmoment: \${escapeHtml(report.scannedAt)}.</p>
  </div>
</body>
</html>\`;
